...
# ╔═╡ 1db59cac-9100-44be-9082-43471cbc8238
begin
	using JuMP
	using Ipopt
	using Plots
...
# ╔═╡ 70630b71-c3bb-4326-b224-eb514330ea17
md"""
In this example, we'll consider five thermal units with the following coefficients:
"""

# ╔═╡ 835b501b-8570-4648-bf74-141c9627d2ba
begin
	a = [3, 4.05, 4.05, 3.99, 3.88] # unit: $/MW^2
	b = [20, 18.07, 15.55, 19.21, 26.18] # unit: $/MW
	c = [100, 98.87, 104.26, 107.21, 95.31] # unit: $
	P_min = [28, 90, 68, 76, 19] # unit: MW
	P_max = [206, 284, 189, 266, 53] # unit: MW
end;

# ╔═╡ 75c345ab-a299-4035-9542-d0cd1c3d6334
md"""
Here is the full model. We implement it as a simple function that takes in the target load and returns the total cost and power outputs per unit if the system is feasible, or throws an error if it's not.
"""

# ╔═╡ f51d2c7e-079c-4c09-b74f-8c372ad257bd
function SolveEconomicDispatch(L_e, verbose=false)
    ...
    if verbose
		@info "Optimal power outputs per unit:"
        for i in 1:5
            @info "P[$i] = $(value.(P[i])) MW"
        end
        @info "Total cost: $(objective_value(model)) \$"
    else
        return objective_value(model), value.(P)
    end
end;

# ╔═╡ eea1c6ce-1f25-4150-b611-b1bd87a51a80
md"""
Now we can easily compute the power outputs and total cost for a given load, in this case, let's say 500 MW.
"""

# ╔═╡ ebea57a6-5693-43b4-ba51-7a2587c81793
SolveEconomicDispatch(500, true);
...
begin
	overload = 50 + sum(P_max);
	
	try
	    SolveEconomicDispatch(overload);
	catch e
	    @info "Error: $(e.msg)"
	end
end;

# ╔═╡ 12bc0ba1-fea9-484a-9144-5ac8c1e60ed3
md"""
Very good! Now, finally we want to reproduce the two graphs of the sections, where we need to perform a load sensitivity analysis. We define a range of (feasible) loads and plot the total operating cost (per hour) and the power outputs of each thermal unit.
"""

# ╔═╡ bac5342f-57e9-4ac4-9d5a-68f9cf2daf4b
begin
	# These are the exact loads used in the book. Interestingly, these numbers came straight from Copilot after entering the first two...
	L_e = [281, 352.7, 424.4, 496.1, 567.8, 639.5, 711.2, 782.9, 854.6, 926.3, 998]
	
	C = zeros(length(L_e))
	P = zeros(length(L_e), 5)
	
	for (i, L) in enumerate(L_e)
	    try
	        C[i], P[i, :] = SolveEconomicDispatch(L)
	    catch e
	        println("Load $L MW is infeasible.")
	        exit(1)
	    end
	end
	
	p1 = plot(L_e, C, xlabel="Load (MW)", ylabel="Cost (\$/h)", title="Total cost per load", legend=false)
	p2 = plot(L_e, P, label=["P1" "P2" "P3" "P4" "P5"], xlabel="Load (MW)", ylabel="Power output (MW)", title="Power output per load", legend=:topleft)
	plot(p1, p2, layout=(2, 1), size=(768, 768))
end
