...
md"""
# Energy storage in wind-integrated DED

In this case, we expect much higher impact of the storage system due to the independent nature of wind energy generation.
We'll re-use the same parameters as in already in [notebook 5](https://lcwllmr.github.io/soroudi2017-jump/05-wind-integrated-ded.html).
Recall that the actual cost due to wind curtailment was 0 in that case, so we might actually not see that high gains in this particular instance.

On the modelling side of things there is nothing new happening here: we simply combine the energy storage from [notebook 6](https://lcwllmr.github.io/soroudi2017-jump/06-energy-storage-in-ded.html) with the wind-integrated DED model.
"""

# ╔═╡ 89162f8c-34c9-11f0-0c62-33f6a61c4dcc
begin
	a = [0.12, 0.17, 0.15, 0.19] # unit: $/MW^2
	b = [14.8, 16.57, 15.55, 16.21] # unit: $/MW
	c = [89, 83, 100, 70] # unit: $
	
	d = [1.2, 2.3, 1.1, 1.1] # unit: kg/MW^2
	e = [-5, -4.24, -2.15, -3.99] # unit: kg/MW
	f = [3, 6.09, 5.69, 6.2] # unit: kg
...
function MakeESSInWindDEDModel()
	...
	@constraint(model, Lcon[t=1:24], Pw[t] + sum(P[i,t] for i in 1:4) + Pd[t] >= L[t] + Pc[t])

	WC = sum(VWC * Pwc[t] for t in 1:24)
	TC = WC + sum(a[i] * P[i,t]^2 + b[i] * P[i,t] + c[i] for i in 1:4, t in 1:24)
    return model, (P, Pw, Pwc, SOC, Pd, Pc, TC)
end;

# ╔═╡ 5a38ec9c-f028-4e05-a604-3cb8e5a79c62
md"""
We inspect the same graphs as before.
"""

# ╔═╡ e5e09112-53e0-4ed0-a0cb-27a666214f2d
begin
	model, (P, Pw, Pwc, SOC, Pd, Pc, TC) = MakeESSInWindDEDModel();
...
begin
	...
	plot(1:24, [L, Ptotal, value.(Pw), value.(SOC)], 
		xlabel="Hour", ylabel="Power", 
		label=["Demand" "Thermal power output" "Wind power output" "Charge"])
end
...
# ╔═╡ de424073-76e2-4297-b532-6ed252d57e3d
md"""
Note that the battery is charging the most in the periods of stronger wind: this is to be expected since the wind is indpendent of demand, obviously.
In general, the battery is charging and discharging for shorter periods of time in comparison to the wind-free case.
We also observe that - due to the low wind in the early morning - the system does not charge and "prepare for the day" as was the case before.

Let's see how the total costs changed.
"""

# ╔═╡ 879f1d7a-cf26-4f03-89ad-0c89346de005
value(TC)

# ╔═╡ dcadcc5f-30f3-47a1-9a45-75db541427ed
md"""
We went down from 226921\$, so the total savings per day are a bit more than 3500\$ per day.
Still not a huge amount, but note that the relative gains are much higher than in the wind-less DED case.
"""
